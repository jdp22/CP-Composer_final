...
class ForceFieldMinimizer(object):
    ...
    def _fix(self, pdb_str, cyclic_chains):
        ...
        if cyclic_chains is not None: # remove OXT and record N, C terminals
            # Use Modeller to remove terminal atoms like OXT
    ...
    def __call__(self, pdb_str, out_path, return_info=True, cyclic_chains=None):
        if '\n' not in pdb_str and pdb_str.lower().endswith(".pdb"):
            with open(pdb_str) as f:
                pdb_str = f.read()

        pdb_fixed, connects = self._fix(pdb_str, cyclic_chains)
